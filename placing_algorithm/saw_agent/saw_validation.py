...
def validation_testing(path, env):
    ...
    while not model.get_env().envs[0].cleared_all:
        ...
        with open('./data_saw/validation_data_gigamodel.json', 'w') as outfile:
            data = model.get_env().envs[0].required_timesteps_dict
            data = {str(k): v for k, v in data.items()}
            json.dump(data, outfile)

        with open('./data_saw/extra_data_gigamodel.json', 'w') as outfile:
            data = model.get_env().envs[0].unfinished_shapes_dict
            data = {str(k): v for k, v in data.items()}
            json.dump(data, outfile)
            
if __name__=='__main__':
    validation_testing('./models/gigamodel/fov_1_test.zip', 'SAWValidation-v0') # replace with model
